(function(){
  ...
  function build(){
    ...
    overlay.setAttribute('aria-label','Rechercher sur Met Road');
    overlay.hidden = true;
    overlay.innerHTML =
      ...
      '<div class="msearch-card" role="document">'+
        '<div class="msearch-head">'+
          ...
          '<input type="text" class="msearch-input" id="msearchInput" placeholder="Rechercher villes, lignes, pays…" autocomplete="off" autocapitalize="off" spellcheck="false" aria-label="Search" aria-controls="msearchList" role="combobox" aria-expanded="true">'+
        ...
        '<div class="msearch-foot"><span><kbd>/</kbd> pour rechercher &middot; <kbd>Esc</kbd> pour fermer &middot; <kbd>&uarr;</kbd><kbd>&darr;</kbd> pour naviguer &middot; <kbd>&crarr;</kbd> pour ouvrir</span></div>'+
      '</div>';
  ...
  function render(query){
    ...
    if(!query.trim()){
      list.innerHTML = '<li class="msearch-hint-row" role="presentation">Commencez à taper une ville, une ligne ou un pays…</li>';
      return;
    }
    if(!results.length){
