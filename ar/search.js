(function(){
  ...
  function build(){
    ...
    overlay.setAttribute('aria-label','البحث في Met Road');
    overlay.hidden = true;
    overlay.innerHTML =
      ...
      '<div class="msearch-card" role="document">'+
        '<div class="msearch-head">'+
          ...
          '<input type="text" class="msearch-input" id="msearchInput" placeholder="ابحث عن مدن وخطوط ودول…" autocomplete="off" autocapitalize="off" spellcheck="false" aria-label="Search" aria-controls="msearchList" role="combobox" aria-expanded="true">'+
        ...
        '<div class="msearch-foot"><span><kbd>/</kbd> للبحث &middot; <kbd>Esc</kbd> للإغلاق &middot; <kbd>&uarr;</kbd><kbd>&darr;</kbd> للتنقل &middot; <kbd>&crarr;</kbd> للفتح</span></div>'+
      '</div>';
  ...
  function render(query){
    ...
    if(!query.trim()){
      list.innerHTML = '<li class="msearch-hint-row" role="presentation">ابدأ بكتابة مدينة أو خط أو دولة…</li>';
      return;
    }
    if(!results.length){
