(function(){
  ...
  function build(){
    ...
    overlay.setAttribute('aria-label','Поиск по Met Road');
    overlay.hidden = true;
    overlay.innerHTML =
      ...
      '<div class="msearch-card" role="document">'+
        '<div class="msearch-head">'+
          ...
          '<input type="text" class="msearch-input" id="msearchInput" placeholder="Поиск городов, линий, стран…" autocomplete="off" autocapitalize="off" spellcheck="false" aria-label="Search" aria-controls="msearchList" role="combobox" aria-expanded="true">'+
        ...
        '<div class="msearch-foot"><span><kbd>/</kbd> — поиск &middot; <kbd>Esc</kbd> — закрыть &middot; <kbd>&uarr;</kbd><kbd>&darr;</kbd> — навигация &middot; <kbd>&crarr;</kbd> — открыть</span></div>'+
      '</div>';
  ...
  function render(query){
    ...
    if(!query.trim()){
      list.innerHTML = '<li class="msearch-hint-row" role="presentation">Начните вводить город, линию или страну…</li>';
      return;
    }
    if(!results.length){
